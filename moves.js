...
MovesApi.prototype.refreshToken = function refreshToken(cb) {
    if(!checkConfig(this.options, ['refreshToken'], cb)) {
        return;
    }
    var url = this.baseUrl + 'oauth/v1/access_token?grant_type=refresh_token' +
            '&refresh_token=' + this.options.refreshToken + '&client_id=' + this.options.clientId + '&client_secret=' +
